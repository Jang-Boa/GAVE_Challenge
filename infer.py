...
from src.loss import DiceLoss  # noqa: F401
from src.resnet import ResNet, ResNetEncoder, BottleNeck, ResNet_Classifier
from src.unetdecoder import UNet
from src.avr import get_result, process_all_images, AVR, load_values
from src.inference_module import inference_avv, inference_optic
...
def main():
    ...
    vessel_weight = os.path.join(args.weight_path, args.vessel_weight)
    encoder_model = ResNet()
    vessel_model = UNet(encoder_model, num_classes=1)
    vessel_model.load_state_dict(torch.load(vessel_weight, weights_only=False))
    vessel_model.to(device)
    vessel_model.eval()
    
    #### Load AV Support Model Weight 
    vessel_support_weight = os.path.join(args.weight_path, args.vessel_support_weight)
    support_model = UNet(encoder_model, num_classes=1)
    support_model.load_state_dict(torch.load(vessel_support_weight, weights_only=False))
    support_model.to(device)
    support_model.eval()
    
    #### Load AV Model Weight
    av_weight = os.path.join(args.weight_path, args.av_weight)
    ...
    av_model.load_state_dict(torch.load(av_weight, weights_only=False))
    av_model.to(device)
    av_model.eval()
    
    #### Load Optic Model Weight
    optic_weight = os.path.join(args.weight_path, args.optic_weight)
    optic_model = torch.load(optic_weight, weights_only=False)
